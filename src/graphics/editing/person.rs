...
pub fn person_listing_system(
    ...
    query: Query<(Entity, &DisplayName, &CreationDate, Option<&BeingEdited>), With<Person>>,
) {
    if !open.0 { return; }
...
pub fn person_editing_system(
    mut ctxs: EguiContexts,
    mut commands: Commands,
    mut query: Query<(Entity, &mut DisplayName, &mut CreationDate, &mut Personality), (With<Person>, With<BeingEdited>)>,
    time: Res<SimulationTime>,
) {
